...
All servers in the project are loaded into memory. Cheburbox builds the cross-server dependency graph (cycles and self-references rejected), reapplies schema validation per server, checks for `hysteria2` `tls.server_name` collisions inside each server, validates `amneziawg` inbound and outbound constraints (non-zero `listen_port`, exactly one CIDR address, valid amnezia protocol and preset on inbounds, required target server and inbound on outbounds), validates `urltest` / `selector` group references against intra-server outbound tags, and verifies that every cross-server outbound's `inbound` tag actually exists on the referenced target server. A cross-server `amneziawg` AllowedIPs collision check then rejects two or more outbounds across the project that claim the same address CIDR against the same `(server, inbound)` target.
...
| 1 | `amneziawg` inbound requires a non-zero `listen_port`, exactly one CIDR address, and (when the `amnezia` block is set) a valid protocol (one of `quic`, `dns`, `dtls`, `stun`, `sip`, `rtp`, `random`) and a valid preset | per-server |
| 1 | `amneziawg` outbound requires a target server, a target inbound tag, and exactly one CIDR address | per-server |
...
| 1 | No two `amneziawg` outbounds across the project claim the same address CIDR against the same `(server, inbound)` target (would produce indistinguishable peers) | cross-server, attributed to every contributing source server |
...
| `FAIL  <server>: inbound "Y": listen_port N is out of range (0-65535)` | An inbound's `listen_port` is negative or exceeds 65535. |
...
| `FAIL  <server>: server "X": amneziawg inbound "Y" requires a non-zero listen_port` | An `amneziawg` inbound has `listen_port: 0`. |
| `FAIL  <server>: server "X": amneziawg inbound "Y" requires exactly one address CIDR, got N` | An `amneziawg` inbound does not have exactly one address entry. |
| `FAIL  <server>: server "X": amneziawg inbound "Y" has invalid address CIDR "C": <parse error>` | An `amneziawg` inbound address does not parse as a CIDR prefix. |
| `FAIL  <server>: server "X": amneziawg inbound "Y" has invalid amnezia protocol "P" (want one of quic, dns, dtls, stun, sip, rtp, random)` | The `amnezia.protocol` value is not in the allowed set. |
| `FAIL  <server>: server "X": amneziawg inbound "Y" has unknown amnezia preset "P"` | The `amnezia.preset` value is not a known preset. |
| `FAIL  <server>: server "X": amneziawg outbound "Y" requires a target server` | An `amneziawg` outbound has an empty `server`. |
| `FAIL  <server>: server "X": amneziawg outbound "Y" requires a target inbound` | An `amneziawg` outbound has an empty `inbound`. |
| `FAIL  <server>: server "X": amneziawg outbound "Y" requires exactly one address CIDR, got N` | An `amneziawg` outbound does not have exactly one address entry. |
| `FAIL  <server>: server "X": amneziawg outbound "Y" has invalid address CIDR "C": <parse error>` | An `amneziawg` outbound address does not parse as a CIDR prefix. |
| `FAIL  <server>: amneziawg outbound address "C" for server "Z" inbound "T" is shared by multiple clients: X, Y` | Two or more `amneziawg` outbounds across the project claim the same address CIDR against the same `(server, inbound)` target. Attributed to every contributing source server. |
